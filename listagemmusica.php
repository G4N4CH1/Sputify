	...
	if(!empty($_POST["busca"])){
       ...
       $tipo_busca= filter_input(INPUT_POST, "tipo_busca", FILTER_SANITIZE_SPECIAL_CHARS);
        ...
        switch( $tipo_busca){
            ...
            default:
                $idBusca= intval($busca);
                $sql_busca=" WHERE id ='$idBusca' OR titulo like '%$busca%' OR  duracao like '%$busca%' ";
                break;
        }
    }
//ordenação
...
$sql = "SELECT id_mus, titulo, duracao, album FROM musicas $sql_busca order by $ordem";
$stmt = $conn->query($sql);
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    href="formulario-alterar-musica.php?id=<?= $row['id_mus'] ?>">
                        <span data-feather="edit"></span>
                        Editar
                    </a>
                </td>
                <td>
                    <a class="btn btn-sm btn-danger" href="excluir-musica.php?id=<?=$row["id_mus"]?>"
